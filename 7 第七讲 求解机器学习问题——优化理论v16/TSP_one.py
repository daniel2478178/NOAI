...
def tournament_selection(population, fitness,k=3, reqiured_num = 3):
    ...
    while current_parent_num < reqiured_num:
        ...
        Survivorind, survivorfitness = tournament[np.argmax(fitness[wherechose])], fitness[np.argmax(fitness[wherechose])]

        #newpop = np.delete(newpop,wherechose,axis = 0)        
        current_parent_num +=1
        #fitness = np.delete(fitness,wherechose)
        fitness = np.append(fitness, survivorfitness)
   
  #      print(newpop.shape,Survivorind.shape)
        newpop = np.append(newpop, Survivorind.reshape(-1,7),axis = 0)
...
def mutate_population(population, mutate_prob):
    ...
    for ind in population:
        if np.random.random() <= mutate_prob:
            ...
            new_ind = swap_mutation(ind.copy())
